...
Left  Hand → Click / Right-Click / Double-Click / Zoom (gesture state machine)
...
# PyAutoGUI settings
pyautogui.PAUSE = 0
pyautogui.FAILSAFE = True          # moving mouse to corner still stops
...
# Scroll parameters
SCROLL_SENSITIVITY = 5          # multiplied by vertical delta
SCROLL_DEADZONE    = 3          # ignore tiny movements (pixels in camera space)

# Zoom (Pinch) — thumb + index extended, track distance changes
ZOOM_ENGAGE_DIST    = 120       # pixels – initial max distance to enter zoom mode
ZOOM_STEP_THRESHOLD = 8         # pixel change per frame to trigger one zoom step

# Thumbs-up strictness
THUMB_UP_MARGIN = 0.05          # thumb tip must be this far (normalised) above knuckle line
...
prev_pinch_dist: Optional[float] = None     # zoom baseline distance
zoom_anchor_dist: Optional[float] = None    # distance when zoom was first entered
...
def handle_right_hand(landmarks, frame_w: int, frame_h: int, frame):
    ...
    if index_ext and middle_ext and not ring_ext and not pinky_ext:
        ...
        if prev_scroll_y is not None:
            delta = prev_scroll_y - mid_y          # positive = hand moved up = scroll up
...
def handle_left_hand(landmarks, frame_w: int, frame_h: int, frame):
    ...
    thumb_up = (
        thumb_tip_y < knuckle_line_y - THUMB_UP_MARGIN  # clearly above ALL knuckles
        and thumb_tip_y < landmarks[THUMB_IP].y          # above its own IP joint
        and thumb_tip_y < landmarks[THUMB_MCP].y         # above its own MCP joint
        and (wrist_y - thumb_tip_y) > 0.15               # substantial distance above wrist
    )

    # ── Peace sign (V-shape): Index + Middle extended, thumb/ring/pinky curled ──
    # Replaces the old L-shape which conflicted with zoom.
    ...
    if detected == GestureState.ZOOM and gesture_state == GestureState.ZOOM:
        if prev_pinch_dist is not None:
            ...
            if abs(delta) > ZOOM_STEP_THRESHOLD:
                try:
                    if delta > 0:
                        pyautogui.hotkey("command", "=")   # zoom in  (macOS)
                    else:
                        pyautogui.hotkey("command", "-")   # zoom out (macOS)
                except pyautogui.FailSafeException:
                    pass
